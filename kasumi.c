 ...
 * InverseFL()
 * Функция InverseFL().
 * Отменяет операцию FL(). Использует <index> для определения
 * соответствующих подключей для использования.
 *---------------------------------------------------------------------*/
static u32 InverseFL(u32 in, int index)
{
    u16 l, r, a, b;
    /* Разбиваем на левую и правую половины */
    l = (u16)(in >> 16);
    r = (u16)(in);
    /* Отменяем операции FL() */
    b = (u16)(r | KLi2[index]);
    l ^= ROL16(b, 1);
    a = (u16)(l & KLi1[index]);
    r ^= ROL16(a, 1);
    /* Объединяем обратно две половины */
    in = (((u32)l) << 16) + r;
 ...
 * InverseFO()
 * Функция InverseFO().
 * Отменяет операцию FO(). Использует <index> для определения
 * соответствующих подключей для использования.
 *---------------------------------------------------------------------*/
static u32 InverseFO(u32 in, int index)
{
    u16 left, right, temp;
    /* Разбиваем входные данные на два 16-битных слова */
    left = (u16)(in >> 16);
    right = (u16)in;
    /* Отменяем те же базовые преобразования три раза */
    temp = FL(left, index);
    temp = InverseFO(temp, index++);
    left ^= temp;
    temp = FL(right, index);
    temp = InverseFO(temp, index++);
    right ^= temp;
 ...
 * InverseFI()
 * Функция InverseFI().
 * Отменяет операцию FI() (рис. 3). Включает в себя таблицы S7 и S9.
 * Преобразует 16-битное значение.
 *---------------------------------------------------------------------*/
static u16 InverseFI(u16 in, u16 subkey)
{
    u16 nine, seven;
    static u16 S7[] = {
        54, 50, 62, 56, 22, 34, 94, 96, 38, 6, 63, 93, 2, 18,123, 33,
    ...
    /* Шестнадцатибитный вход разбивается на две неравные половины, *
     * девять бит и семь бит - как и подключ. */
    nine = (u16)(in >> 7);
    seven = (u16)(in & 0x7F);
    /* Теперь выполняем различные операции в обратном порядке */
    seven = (u16)(S7[seven] ^ (nine & 0x7F));
    nine = (u16)(S9[nine] ^ seven);
    nine ^= (subkey & 0x1FF);
    seven ^= (subkey >> 9);
...
/*---------------------------------------------------------------------
 * InverseKasumi()
 * Расшифровывает 64-битные входные данные с использованием алгоритма Kasumi.
 *---------------------------------------------------------------------*/
void InverseKasumi(u32 *data)
{
    u32 left, right, temp;

    int n = 7;
    /* Начнем с получения данных в два 32-битных слова (в правильном порядке) */
    left = data[0];
    right = data[1];
    do
    {
        temp = InverseFO(left, n);
        temp = InverseFL(temp, n--);
        right ^= temp;

        temp = InverseFL(right, n);
        temp = InverseFO(temp, n--);
        left ^= temp;
